...
from .config import BridgeConfig, Route
...
def resolve_route(config: BridgeConfig, model_id: str) -> RouteResolution:
    ...
    if matches:
        ...
        if len(best) > 1:
            ...
            raise ConfigurationError(f"Multiple bridge routes match {model_id!r}: {ids}")
        route = best[0]
    ...
    raise ConfigurationError(
        "No Bedrock bridge route resolved. Set BEDROCK_BRIDGE_PROVIDER and "
        "BEDROCK_BRIDGE_MODEL or create bedmock.json."
    )
