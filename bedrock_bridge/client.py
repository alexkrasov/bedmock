...
class BedrockRuntimeClient:
    ...
    def apply_guardrail(self, **kwargs: Any) -> None:
        raise UnsupportedOperationException(
            ...
            "provider-independent bridge equivalent.",
    ...
    def _raise_async_unsupported(self, operation_name: str) -> None:
        raise UnsupportedOperationException(
            ...
            "input/output locations; they are explicit non-goals for bedrock_bridge.",
    ...
    def _context(self, operation_name: str, model_id: str) -> OperationContext:
        route = resolve_route(self.bridge_config, model_id)
        provider = load_provider_profile(
            route.provider_id,
            profile_path=self.bridge_config.provider_profile_path,
            overrides=self.bridge_config.provider_overrides,
        )
    ...
    def _transport(self, provider: ProviderProfile) -> ProviderTransport:
        ...
        transport = build_transport(
            ...
            connect_timeout_seconds=self.bridge_config.connect_timeout_seconds,
            max_retries=self.bridge_config.max_retries,
            verify=self.verify,
            strict_parameters=self.bridge_config.strict_parameters,
